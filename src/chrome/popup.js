...
function init() {
  ...
  chrome.runtime.sendMessage({ get_prefs: 'all' }, (response) => {
    if (chrome.runtime.lastError) {
      console.error('Error getting prefs:', chrome.runtime.lastError.message);
      return;
    }
    if (response && response.error) {
  ...
  autoEle.addEventListener('click', function () {
    setAVIMConfig('method', 0);
  });
  telexEle.addEventListener('click', function () {
    setAVIMConfig('method', 1);
  });
  vniEle.addEventListener('click', function () {
    setAVIMConfig('method', 2);
  });
  viqrEle.addEventListener('click', function () {
    setAVIMConfig('method', 3);
  });
  viqrStarEle.addEventListener('click', function () {
    setAVIMConfig('method', 4);
  });
